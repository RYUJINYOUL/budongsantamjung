...
export const PANEL_SECTION_LABEL =
    'text-xs font-extrabold text-slate-800 tracking-tight';

export const PANEL_SECTION_DESC =
    'text-xs text-slate-600 font-bold mt-0.5';
...
export const PANEL_INPUT =
    'flex-1 min-w-0 bg-transparent border-none outline-none text-xs font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-semibold';

export const PANEL_HINT = 'mt-1.5 text-[10px] font-bold text-emerald-600 tabular-nums';

export const PANEL_SELECT =
    'w-full bg-slate-50/80 border border-slate-200/70 rounded-xl px-3.5 py-2.5 text-xs font-bold text-slate-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/10 transition-all appearance-none';

export function panelChoiceBtn(selected: boolean) {
    return [
        'py-2.5 rounded-xl text-xs font-extrabold border transition-all',
        selected
...
/** 필터 칩 — 활성 시 옅은 녹색 (철도·정비 등) */
export function panelChoiceBtnSoft(selected: boolean) {
    return [
        'py-2.5 rounded-xl text-xs font-extrabold border transition-all',
        selected
            ? 'border-emerald-300 bg-emerald-50 text-emerald-800 shadow-sm shadow-emerald-500/10'
            : 'border-slate-200/80 bg-white text-slate-600 hover:border-emerald-200 hover:bg-emerald-50/40 hover:text-emerald-700',
    ].join(' ');
}
