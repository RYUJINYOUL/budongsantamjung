...
import { 
    ...
    HelpCircle, CheckSquare, RefreshCw, Layers2, Eye, Shield, 
    ...
    return (
        <div className="bg-white/5 p-4 rounded-[20px] border border-white/10">
            <div className="flex justify-between items-center mb-1">
                ...
                <span style={{ color }} className="text-xs font-black">{score} / {max}점</span>
            </div>
            ...
            {reason && <div className="text-xs text-white/50 mt-2 leading-relaxed">{reason}</div>}
        </div>
    );
};
...
export default function AiReportView({ ai, mergedData, onTriggerAnalysis, isCheckingAccess }: any) {
    ...
    const inDepthCategories: Record<string, any> = {
        ...
        'landAndBuildingValue': { icon: Layers2, label: '토지 + 건물 분리 가치', color: '#c5dedd' },
    ...
    const renderValuationLedgerSection = () => {
        ...
        const multiplierTile = (indexStr: string, name: string, factor: number, desc: string, customDescElement?: React.ReactNode) => {
            ...
            return (
                <div className="p-3 bg-white/20 border border-white/5 rounded-2xl flex flex-col gap-2">
                    <div className="flex justify-between items-start">
                        ...
                        <div style={{ backgroundColor: badgeColor, color: textColor, borderColor: strokeColor }} className="px-2 py-1 rounded-lg border text-[11px] font-black">
                            {factor.toFixed(3)}x
                        </div>
                    </div>
            ...
            <div className="p-6 bg-[#0f172a]/55 border border-[#c5dedd]/20 rounded-[40px] shadow-[0_0_25px_rgba(197,222,221,0.04)]">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    {multiplierTile(
                        ...
                        '시점 보정 (Time)',
                        avgTimeFactor,
                        '한국부동산원 지가변동률 지수 및 거래월 기준 시계열 변동 반영'
                    )}
                    {hosaeAdj && (
                        multiplierTile(
                            '⑥',
                            '호재 보정 (Hosae)',
                            hosaeAdj.applied ? (hosaeAdj.factor || 1.0) : 1.0,
                            hosaeAdj.applied ? '' : '인근 호재 미감지 — 보정 없음',
                            hosaeAdj.applied ? renderHosaeDetails() : undefined
                        )
                    )}
                </div>
                ...
                <div className="p-4 bg-white/5 border border-white/5 rounded-2xl flex flex-col gap-2">
                    {marketSummary.length === 0 ? (
                        ...
                        marketSummary.map((textVal: any, idx: number) => {
                            ...
                            const hasEmoji = text.startsWith('⚠') || text.startsWith('✅') || text.startsWith('🔨') || text.startsWith('📊');
            ...
            {Object.keys(priceReas).length > 0 && (
                <div className="p-6 bg-[#0f172a] rounded-[40px] border border-[#c5dedd]/20 shadow-[0_0_25px_rgba(197,222,221,0.04)] flex flex-col gap-4">
                    ...
                    {isApartment && priceReas.marketSummaryComment && (
                        priceSubCard(BarChart3, '시장 분석 코멘트', String(priceReas.marketSummaryComment), '#90e0ef')
                    )}
                </div>
            )}
                ...
                <div className="p-6 bg-[#0f172a]/55 border border-[#fad2e1]/20 rounded-[40px] shadow-[0_0_25px_rgba(250,210,225,0.04)]">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="p-2 bg-[#fad2e1]/12 border border-[#fad2e1]/30 rounded-xl">
                            <Layers2 className="w-4 h-4 text-[#fad2e1]" />
                        </div>
