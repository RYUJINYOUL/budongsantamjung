...
export default function DetectiveSummaryView({
    ...
    const [calcExclusiveArea, setCalcExclusiveArea] = useState(0);
    ...
    const getRegionalTradeCount = (regionalTrades: any[], typeKeyword: string): string => {
        ...
        for (const group of regionalTrades) {
            ...
            if (type.replace(/\s+/g, '').includes(typeKeyword.replace(/\s+/g, ''))) {
                if (typeof group.latestMonthCount === 'number') {
                    return String(group.latestMonthCount);
                }
                const data = group.data || [];
                if (!data.length) return '0';
                let latestMonth = '';
                for (const trade of data) {
                    const year = trade?.dealYear ?? trade?.deal_year;
                    const month = String(trade?.dealMonth ?? trade?.deal_month ?? '').padStart(2, '0');
                    if (!year || !month) continue;
                    const key = `${year}${month}`;
                    if (key > latestMonth) latestMonth = key;
                }
                if (!latestMonth) return String(data.length);
                const latestCount = data.filter((trade: any) => {
                    const year = trade?.dealYear ?? trade?.deal_year;
                    const month = String(trade?.dealMonth ?? trade?.deal_month ?? '').padStart(2, '0');
                    return `${year}${month}` === latestMonth;
                }).length;
                return String(latestCount);
            }
        }
        return '0';
    };

    const getTargetComplexLatestMonthSaleCount = (trades: any[], stats?: { latestMonthCount?: number }): string => {
        if (typeof stats?.latestMonthCount === 'number') {
            return String(stats.latestMonthCount);
        }
        const saleTrades = (trades || []).filter((trade) => trade && trade._isRent !== true);
        if (!saleTrades.length) return '0';

        let latestMonth = '';
        for (const trade of saleTrades) {
            const year = trade?.dealYear ?? trade?.deal_year;
            const month = String(trade?.dealMonth ?? trade?.deal_month ?? '').padStart(2, '0');
            if (!year || !month) continue;
            const key = `${year}${month}`;
        ...
        const latestCount = saleTrades.filter((trade) => {
            const year = trade?.dealYear ?? trade?.deal_year;
            const month = String(trade?.dealMonth ?? trade?.deal_month ?? '').padStart(2, '0');
            return `${year}${month}` === latestMonth;
        }).length;
    ...
    const apartmentAreaMapping = useMemo(
        () => parseAreaMapping(rawData?.vitals?.building?.areaMapping),
        [rawData?.vitals?.building?.areaMapping]
    );

    // ──────────────────────────────────────────
    // 📊 6개월 실거래가 (areaMapping 있으면 타입별, 없으면 전용㎡)
    // ──────────────────────────────────────────
    const pyungSummary = useMemo(() => {
        ...
        for (const trade of backendTargetTrades) {
            ...
            const groupKey = resolveAreaGroupKey(areaVal, apartmentAreaMapping);
            if (groupKey <= 0) continue;

            if (!areaTradesList[groupKey]) areaTradesList[groupKey] = [];
            areaTradesList[groupKey].push(trade);
        }

        return Object.entries(areaTradesList).map(([areaStr, trades]) => {
            const exclusiveArea = parseFloat(areaStr);
    ...
    }, [backendTargetTrades, apartmentAreaMapping]);
    ...
    const openCalculator = (exclusiveArea: number, recentPrice: number) => {
        setCalcExclusiveArea(exclusiveArea);
                        ...
                        >
                            ...
                            <div className="px-6 py-4 flex justify-between items-center border-b border-white/5 shrink-0">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <span className="text-white font-bold text-[17px]">{formatAreaTypeHeader(calcExclusiveArea, apartmentAreaMapping)} 매매 간편 계산기</span>
                                </div>
            ...
            <div className="p-6 bg-gradient-to-br from-sky-950/30 to-slate-900/50 border border-sky-500/20 rounded-[32px] shadow-[0_24px_50px_-12px_rgba(0,0,0,0.7)] backdrop-blur-2xl transition-all duration-300 hover:border-sky-500/30">
                ...
                <div className="space-y-4">
                    {(shortsMode ? pyungSummary.slice(0, 3) : pyungSummary).map((item) => {
                        ...
                        return (
                            <div key={item.exclusiveArea} className="p-5 bg-white/[0.03] border border-white/[0.05] hover:border-white/[0.1] rounded-2xl space-y-4 transition-all duration-300 hover:bg-white/[0.05]">
                                <div className="flex justify-between items-center">
                                    <span className="text-sky-300 text-base font-black tracking-tight">
                                        {formatAreaTypeHeader(item.exclusiveArea, apartmentAreaMapping)}
                                    </span>
                                    {!shortsMode && (
                                        <button
                                            onClick={() => openCalculator(item.exclusiveArea, item.recentPrice)}
    ...
    function renderApartmentMarketSummarySection() {
        ...
        const targetComplexTradeStats = rawData.vitals?.targetComplexTradeStats;
            ...
            <div className="p-6 bg-[#13131a]/80 border border-white/[0.08] rounded-[32px] shadow-[0_24px_50px_-12px_rgba(0,0,0,0.7)] backdrop-blur-2xl space-y-6">
                ...
                <div className="p-5 bg-white/[0.03] border border-white/[0.05] rounded-[24px] space-y-4 transition-all">
                    <div className="flex justify-between items-center text-xs font-semibold">
                        <span className="text-white/60">해당 단지 매매 거래량 (최근 1개월)</span>
                        <span className="text-sky-400 font-black text-sm">{getTargetComplexLatestMonthSaleCount(backendTargetTrades, targetComplexTradeStats)}건</span>
                    </div>
                </div>
            </div>
        );
