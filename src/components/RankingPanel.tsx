...
export default function RankingPanel({ onResultsChange, urlPrefill, hideHeader = false }: RankingPanelProps) {
  ...
  useEffect(() => {
    ...
    const raw = localStorage.getItem('recent_ranking_searches');
    if (raw) {
      try {
  ...
  const saveSearchToHistory = (code: string, name: string, min: number | null, max: number | null, type: string) => {
    if (typeof window === 'undefined') return;
    const item = { sigunguCd: code, sigunguName: name, minPrice: min, maxPrice: max, type, timestamp: Date.now() };
    
    setRecentSearches(prev => {
      const filtered = prev.filter(x => !(x.sigunguCd === code && x.type === type && x.minPrice === min && x.maxPrice === max));
      const next = [item, ...filtered].slice(0, 6);
      localStorage.setItem('recent_ranking_searches', JSON.stringify(next));
      return next;
    });
  };
  ...
  const handleRankingSearch = async (
    ...
    try {
      ...
      if (data.success) {
        ...
        const korName = data.sigunguName || searchQ || code;

        // 검색 이력 저장
        saveSearchToHistory(code, korName, finalMin || null, finalMax === 99999999 ? null : finalMax, type);

        // 검색어 입력창 복원
        if (!searchQ && data.sigunguName) {
          setSearchQ(data.sigunguName);
        }
        ...
        if (!skipUrlUpdate) {
          ...
          params.set('sigunguName', korName);
      ...
      {!hideHeader && (
        <div className={PAGE_SUBHEADER}>
          ...
          <div className="flex w-full mb-4 bg-slate-100 p-1 rounded-xl shadow-inner border border-slate-200">
            <button 
              className="flex-1 py-2 text-sm font-extrabold text-emerald-800 bg-emerald-50 rounded-lg shadow-sm"
            >
              AI 랭킹
            </button>
            <button 
          ...
          {TABS.map(tab => {
            ...
            return (
              ...
              >
                ...
                />
                <span className={`text-[11px] font-extrabold leading-none ${isActive ? 'text-emerald-700' : 'text-slate-600'}`}>{tab.label}</span>
              </button>
            );
          })}
        </div>
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-3">
            ...
            <div className="flex-1 min-w-[150px] flex items-center gap-1.5">
              <input
                ...
                className={`${PANEL_SELECT} flex-1 min-w-0 text-center`}
              />
              <span className="text-slate-400 text-xs shrink-0 font-extrabold">~</span>
              <input
                type="number"
                placeholder="최대 (만원)"
                value={maxBudget || ''}
            ...
            <div className="flex flex-wrap gap-1.5">
              {BUDGET_CHIPS.map(chip => (
                <button
                  ...
                  className={`${panelChoiceBtn(false)} px-2.5 py-1.5`}
                >
                  {chip.label} 이하
                </button>
              ))}
            ...
            {recentSearches.length > 0 && (
              <div className="mt-4 border-t border-slate-100 pt-3.5">
                <h4 className="text-[11px] font-extrabold text-slate-500 mb-2">
                  ⏱️ 최근 조회 기록
                </h4>
                <div className="flex flex-wrap gap-1.5">
                  {recentSearches.map((s, idx) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setSearchQ(s.sigunguName);
                        setAptSigunguCd(s.sigunguCd);
                        setMinBudget(s.minPrice);
                        setMaxBudget(s.maxPrice);
                        setRankingType(s.type);
                        handleRankingSearch(s.sigunguCd, s.minPrice || undefined, s.maxPrice || undefined, s.type);
                      }}
                      className="px-2.5 py-1.5 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 rounded-lg text-[11px] font-bold text-slate-700 transition-colors border border-slate-200/50 flex items-center gap-1 cursor-pointer"
                    >
                      <span className="font-extrabold">{s.sigunguName}</span>
                      <span className="text-[10px] text-slate-500 font-semibold">
                        ({s.type === 'apartment' ? '아파트' : s.type === 'land' ? '토지' : '빌딩'})
                      </span>
                    </button>
                  ))}
                </div>
          ...
          <section className="mt-4">
            ...
            {isSearchingRank ? (
              ...
              <div className="space-y-3">
                <div className="text-[10px] text-slate-400 font-bold mb-2 text-right">
                  {activeTab.sortDesc}
                </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {type === 'land' && item.jimok && (
              <span className="text-[10px] px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded-md border border-emerald-100/50 font-bold">지목: {item.jimok}</span>
            )}
            {type === 'building' && item.yieldRate === null && (
              <span className="text-[9px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded-md border border-slate-200 font-bold">수익률 미산출</span>
            )}
          </div>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {type === 'apartment' && (
                <>
                  ...
                  <span className="text-[10px] font-bold text-slate-500">해당 평형 매매 {item.saleCount}건</span>
                </>
              )}

              {/* 토지: 면적 */}
              {type === 'land' && item.targetArea > 0 && (
                <span className="text-[10px] font-bold text-slate-500">{item.targetArea.toLocaleString()}㎡</span>
              )}
              ...
              {type === 'building' && item.yieldRate !== null && (
                <>
                  ...
                  {item.noi && item.noi > 0 && (
                    <span className="text-[10px] font-bold text-slate-500">연 NOI {formatEok(item.noi)}</span>
                  )}
                </>
              )}
            </div>
