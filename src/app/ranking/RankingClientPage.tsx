...
export default function RankingClientPage() {
  ...
  return (
    <div className="detective-bg min-h-screen text-slate-900 relative font-sans antialiased">
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-[minmax(0,25%)_minmax(0,75%)] h-screen relative z-10 w-full overflow-hidden lg:pl-16">
        ...
        {/* 좌측 패널 */}
        <div className={`w-full flex flex-col bg-gradient-to-b from-white to-slate-50/30 min-w-0 z-20 overflow-hidden lg:h-full lg:min-h-0 ${showMobileMap ? 'max-lg:shrink-0 max-lg:h-auto border-b lg:border-b-0 border-slate-200/50 shadow-sm lg:shadow-none' : 'flex-1 min-h-0'}`}>
          {/* 공통 상단 헤더 (활성화된 탭에 맞춰 타이틀을 랭킹 / 지역 비교로 변경) */}
          <header className={PAGE_STICKY_HEADER}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 lg:hidden" />
                <h1 className={PAGE_HEADER_TITLE}>
                  {activeSubPanel === 'ranking' ? 'AI 랭킹' : '지역 비교 브리핑'}
                </h1>
              </div>
              {/* 모바일 지도/목록 토글 버튼 */}
              {isMobile && (
                <button
                  ...
                  className="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1 rounded-xl font-bold text-xs tracking-wide shadow-sm transition-all active:scale-95"
                >
          ...
          <div className="px-4 lg:px-5 pb-2 shrink-0">
            <p className="text-[11px] font-semibold text-slate-400">
              지역별 투자별 순위를 확인해보세요
            </p>

            {/* 탭 네비게이션 (바탕색을 은은한 녹색 계열 bg-emerald-50/60 으로 적용) */}
            <div className="flex w-full mt-3 bg-slate-100 p-1 rounded-xl shadow-inner border border-slate-200">
              <button
                onClick={() => {
                  const params = new URLSearchParams(searchParams.toString());
                ...
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${
                  activeSubPanel === 'ranking'
                    ? 'text-emerald-700 bg-emerald-50/60 shadow-sm font-black'
          ...
          <div className="h-full flex flex-col w-full">
            <div className="flex-1 relative">
              ...
              <KakaoMap
                ...
                isRankingMode={activeSubPanel === 'ranking'}
